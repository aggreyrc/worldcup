  ...
  return (
    <div className="flex items-start gap-3 py-2">
      ...
      <div className="text-sm">
        <span className="text-slate-200 font-medium">{event.player || event.type || "Match event"}</span>
        {event.assist && <span className="text-slate-500 ml-1.5 text-xs">(assist: {event.assist})</span>}
        <div className="text-slate-500 text-xs mt-0.5">{[event.team, event.detail].filter(Boolean).join(" · ")}</div>
      </div>
    </div>
  );
}

export default function MatchTimeline({ events = [], compact = false }) {
  if (!events.length) {
    return <p className="text-slate-500 text-sm py-4">No events yet.</p>;
  }

  if (compact) {
    return <div className="divide-y divide-white/5">{events.map((e, i) => <EventRow key={i} event={e} />)}</div>;
  }

  const firstHalf = events.filter((e) => eventMinute(e) <= 45);
  const secondHalf = events.filter((e) => eventMinute(e) > 45 && eventMinute(e) <= 90);
  const extra = events.filter((e) => eventMinute(e) > 90);
